...
class FullDescriptives:
    ...
    def __str__(self):
        ...
        s+= 'Missing'+str(self.missing )+'\n'
        s+= 'N:'+str(self.N )+'\n'
        s+= 'Sum:'+str(self.sum )+'\n'
        s+= 'Mean:'+str(self.mean )+'\n'
        s+= 'Sumsq:'+str(self.sumsquares )+'\n'
        s+= 'Sortlist:'+str(self.sortlist )+'\n'
        s+= 'Min:'+str(self.minimum)+'\n'
        s+= 'Max:'+str(self.maximum)+'\n'
        s+= 'Range:'+str(self.range)+'\n'
        s+= 'Harmean:'+str(self.harmmean)+'\n'
        s+= 'Median:'+str(self.median)+'\n'
        s+= 'Mad:'+str(self.mad)+'\n'
        s+= 'Numberuniques:'+str(self.numberuniques)+'\n'
        s+= 'Ssdev:'+str(self.ssdevs)+'\n'
        s+= 'Geomean:'+str(self.geomean)+'\n'
        s+= 'Variance:'+str(self.variance)+'\n'
        s+= 'Stddev:'+str(self.stddev)+'\n'
        s+= 'Coefcvar:'+str(self.coeffvar)+'\n'
        s+= 'Sknewness:'+str(self.skewness )+'\n'
        s+= 'Kurtosis:'+str(self.kurtosis)+'\n'
        s+= 'Stderr:'+str(self.stderr)+'\n'
...
class TwoSampleTests:
    ...
    def TwoSampleSignTest(self, data1, data2):
        ...
        if (self.d1.N != self.d2.N):
            ...
            self.nplus=map(higher,data1,data2).count(1)
            self.nminus=map(lower,data1,data2).count(1)
            self.ntotal=self.nplus-self.nminus
            mean=self.d1.N / 2
            sd = math.sqrt(mean)
            self.z = (self.nplus-mean)/sd
            self.prob = erfcc(abs(self.z)/1.4142136)
