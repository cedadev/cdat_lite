...
def __myGetAxisWeights(x, i, axisoptions=None):
    """ Assume x is an MV duh! and get the weights associated with axis index i.
    ...
    if x.getAxis(i).isLatitude():
        Lataxis = x.getAxis(i)
        xgr = cdms.createGenericGrid(Lataxis[:], Numeric.array(range(1), Numeric.Float), latBounds=Lataxis.getBounds())
        xlatwt, xlonwt = xgr.getWeights()
        return xlatwt
    elif x.getAxis(i).isLongitude():
        Lonaxis = x.getAxis(i)
        xgr = cdms.createGenericGrid(Numeric.array(range(1), Numeric.Float), Lonaxis[:], lonBounds=Lonaxis.getBounds())
        xlatwt, xlonwt = xgr.getWeights()
        return xlonwt
    else:
	    ...
	    ax=x.getAxis(i)
            if axisoptions is not None:
                axo=cdms.orderparse(axisoptions)
                if (i in axo ) or (ax.isTime() and 't' in axo) or (ax.isLevel() and 'z' in axo) or ('('+ax.id+')' in axo):
                    ...
                    axis_wts=Numeric.ones(x.getAxis(i)[:].shape)
            else:
...
def getAxisWeight(x, i):
    ...
    is returned as an MV!
    """
    try:
        wts = __myGetAxisWeights(x, i)
        axis_i = x.getAxis(i)
        wtTV = cdms.createVariable(wts, axes=[axis_i])
    except:
...
def getAxisWeightByName(x, name):
    ...
    try:
        ...
        wtTV = cdms.createVariable(wts, axes=[axis_name])
    except:
        raise AveragerError, 'Could not get weights.'
    return wtTV
...
def area_weights(ds,axisoptions=None):
    ...
    if seenlat and seenlon:
        ...
        #wt = Numeric.outerproduct(__myGetAxisWeights(ds,Lataxisindex), __myGetAxisWeights(ds,Lonaxisindex))
        dsgr = ds.getGrid()
        latwts, lonwts = dsgr.getWeights()
        wt = Numeric.outerproduct(Numeric.array(latwts), Numeric.array(lonwts))
        ...
        for i in range(len(s)-1, -1, -1):
            if (i != Lataxisindex) and (i != Lonaxisindex):
                ...
                wt = Numeric.resize(wt, new_wtshape)
                ...
                newaxiswt = Numeric.resize(newaxiswt, tuple(new_newaxiswt_shape))
                wt =  wt * newaxiswt
            # end of if (i != Lataxisindex) and (i != Lonaxisindex):
        # end of for i in range(len(s)):
        wt = cdms.createVariable(MA.masked_array(wt, MA.getmask(ds)), axes=ds.getAxisList())
        ...
        return cdms.createVariable(MA.masked_array(wt, MA.getmask(ds)), axes=ds.getAxisList())
...
def __check_each_weight_option(x, ax, index, wtopt):
    ...
    if MV.isMaskedVariable(wtopt):
        ...
        #
        try:
            if __DEBUG__: print '****Order of axes in wtopt (originally) = ', MV.getorder(wtopt)
            wtopt = wtopt(order=ax)
            if __DEBUG__: print '****Order of axes in wtopt    (finally) = ', MV.getorder(wtopt)
        except:
            raise AveragerError, 'The MV passed does not have an axis matching '+str(ax)
        # end of try:
        if MA.shape(wtopt(order=ax)[:])[0] != len(x.getAxis(index)[:]):
            # We have an MV whose length is not the same as that of the corresponding axis for x
            raise AveragerError, \
                  'The Masked Variable passed does not match the length of axis ' + str(ax)
        # end of if MA.shape(wtopt(order=ax)[:])[0] != len(x.getAxis(index)[:]):
        if len(MA.shape(wtopt)) == 1:
            #
            # Coerce the MV to Numeric type..... only if the wtopt is 1-d!!
            #
            if __DEBUG__: print 'Returning a numeric array from MV!!'
            wtopt = MA.filled(wtopt)
        # end of if len(MA.shape(wtopt)) == 1:
    elif MA.isMA(wtopt):
        #
        # wtopt is an MA
        #
        if __DEBUG__: print 'I have a Masked Array of rank', MA.rank(wtopt)
        #
        if MA.rank(wtopt) > 1:
            raise AveragerError, \
                 'Error: The Masked Array of more than 1 dimension lacks sufficient information'
        # end of if MA.rank(wtopt) > 1:
        if MA.shape(wtopt)[0] != len(x.getAxis(index)[:]):
            raise AveragerError, 'Error: Axis is of length %d, weights passed of length %d.' %\
            (len(x.getAxis(index)[:]), MA.shape(wtopt)[0])
        # end of if MA.shape(wtopt)[0] != len(x.getAxis(index)[:]):
        #
        # Coerce the MA to Numeric type.....
        #
        if __DEBUG__: print 'Returning a numeric array from MA!!'
        wtopt = wtopt.filled()
    elif isinstance(wtopt, Numeric.ArrayType):
        #
        # wtopt is a Numeric Array
        #
        if __DEBUG__: print 'I have a Numeric Array of rank', len(Numeric.shape(wtopt))
        #
        if len(Numeric.shape(wtopt)) > 1:
            raise AveragerError, \
            'Error: The Numeric Array of more than 1 dimension lacks sufficient information'
        if Numeric.shape(wtopt)[0] != len(x.getAxis(index)[:]):
            raise AveragerError, 'Error: Axis is of length %d, weights passed of length %d' % \
            (len(x.getAxis(index)[:]), Numeric.shape(wtopt)[0])
        ...
        if wtopt.lower() in ['equal','unweighted']:
            ...
            # NOTE: THIS FUNCTION CAN BE CHANGED WHEN BOB PUTS THE FUNCTION getAxisWeights() INTO cdms
    ...
    # end of if MV.isMaskedVariable(wtopt):
    return wtopt
...
def __check_weightoptions(x, axisoptions, weightoptions):
    ...
    #
    axisindex = cdms.order2index(x.getAxisList(), axisoptions)
    if __DEBUG__: print 'This should be 0,1,2... etc. is it? ', axisindex
    #
    axislist = cdms.orderparse(axisoptions)
    if __DEBUG__: print 'axislist = ', axislist
    #
    if not isinstance(weightoptions, types.ListType):
        #
        # We have either 1 axis only or multiple axes and one MV of weights
        #
        if MV.isMaskedVariable(weightoptions):
            #
            # Weight passed is an MV. Probably OK
            #
            try:
...
##                 # end of if x.shape != weightoptions.shape:
            ...
            # figure out if it is an array (Numeric or MA) or 'equal' or 'generate' or something else
            #
            ...
            #
            # More than 1 axis to reduce over, and not an MV of the same dimensionality as x!
            #
...
def _check_MA_weight_options(weightoptions, shx, N_axisopt):
    ...
    Inputs:
      ...
      shx           : Is the shape of the reordered MA
    """
    #
    __DEBUG__ = 0
    #
    ...
    for i in range(len(weightoptions)):
        ...
        if isinstance(wt, types.StringType):
            ...
            if wt in ['weighted', 'generate']:
                raise AveragerError, \
                      'Cannot generate weights for an MA. %s is not a valid option when you do not pass an MV. ' % wt
            elif (wt in ['equal', 'unweighted']):
                weightoptions[i] = Numeric.ones(tuple([shx[i]]), Numeric.Float)
            # end of if wt in ['weighted', 'generate']:
        elif isinstance(wt, Numeric.ArrayType) :
            if __DEBUG__: print 'Numeric Array weights'
            if (wt.shape != shx) and (Numeric.rank(wt) == 1 and len(wt) != shx[i]):
                raise AveragerError, 'The passed weight is not of the appropriate shape'
            # end of if (wt.shape != shx) or (MA.rank(wt) == 1 and len(wt) != shx[i]):
        elif MA.isMA(wt) :
            if __DEBUG__: print 'MA Array weights'
            if __DEBUG__: print 'wt.shape = ', wt.shape
            if __DEBUG__: print 'MA.rank(wt) = ', MA.rank(wt)
            if __DEBUG__: print 'len(wt) = ', len(wt)
            if __DEBUG__: print 'shx[i] = ', shx[i]
            if (wt.shape != shx) and (MA.rank(wt) == 1 and len(wt) != shx[i]):
...
def _combine_weights(x, weightoptions):
    ...
    if isinstance(weightoptions, types.ListType):
        ...
        if n_dimstoadd < 0 :
            ...
            #
            wt_init = Numeric.ones(init_shape, Numeric.Float)
            #
        ...
        if wgt in ['equal', 'unweighted']:
            wgt = Numeric.ones(xsh[-dim], Numeric.Float)
        else:
        ...
        if not wt_init:
            ...
            wt_init = Numeric.resize(wt_init, tuple(newshape))
            ...
            wgt = Numeric.resize(wgt, tuple(wgtsh))
            wt_init = wt_init * wgt
        ...
        for i in range(len(weightoptions)-2, -1, -1):
            ...
            if dim_wt in ['equal', 'unweighted']:
                dim_wt = Numeric.ones(xsh[i], Numeric.Float)
            # end of if dim_wt in ['equal', 'weighted']:
            if __DEBUG__: print 'At step ', i, dim_wt
            #wt_init = Numeric.outerproduct(wt_init, dim_wt)
            ...
            dim_wt = Numeric.resize(dim_wt, tuple(dim_wtsh))            
        ...
        #
        if MV.isMaskedVariable(x): wt_init = MV.array(wt_init, axes=x.getAxisList())
        weightoptions = [wt_init]
        #
        if __DEBUG__: print 'Are my weight and area_weights the same?', MA.allclose(weightoptions[0], area_weights(x))
    elif MV.isMaskedVariable(weightoptions):
        if __DEBUG__: print 'weightoptions is an MV. Nothing to do?'
    elif MA.isMaskedArray(V) and not MV.isMaskedVariable(V):
    ...
    for i in range(len(axisoption)):
        ...
        if not axis:
            ...
            try:
                ...
                raise AveragerError, \
                      '%s is not a valid axis for an MA. You must pass an MV' % axis
            # end of try:
...
def sum_engine(x, wts):
    ...
    1-d wts are always Numeric arrays or 'equal' Multi-dimensional arrays are
    always MV's
    
    Inputs:
        x   : the input array (can be MV or MA)
        wts : the input weight array (Numeric array or MV or 'equal')
    ...
    if MV.isMaskedVariable(wts) or isinstance(wts, Numeric.ArrayType):
        ...
        return None
    # end of if MV.isMaskedVariable(wts) or isinstance(wts, Numeric.ArrayType):
...
Where V is an array. It can be an array of Numeric, MA or MV type. In each case
...
-----
    ...
    #
    # Note: Further checking on weights is done later - in the MA & MV sections also.
    #
    ...
    #
    # Account for MV, MA or Numeric arrays sent in by users. Return result of same type.
    #
    #
    # Case 1. Numeric array
    #         Convert Numeric array to MA and remember it using _NUM_FLAG so you
    #         can convert the answer to Numeric array before returning
    #
    if isinstance(V, Numeric.ArrayType):
        if __DEBUG__: print 'Converting to MA so I can do an MA.average or sum'
        V = MA.array(V)
        _NUM_FLAG = 1
    else:
        _NUM_FLAG = 0
    # end of if isinstance(V, Numeric.ArrayType):
    #
    #
    # Case 2. Masked Array (MA)
    #
    if MA.isMaskedArray(V) and not MV.isMaskedVariable(V):     
        #
        # The passed array is an MA
        #
        if __DEBUG__: print 'Entered MA only....'
        if __DEBUG__: print '!!!!!!Checking weights for MA', weights
        #
        #
        ...
        #
        axis = _check_MA_axisoptions(axis, MA.rank(V))
        #
        ...
        if action == 'average':
            ...
            for i in range(len(axis)):
                ...
                #
                if  isinstance(weights[i] , types.StringType) or (not weights[i]):
                    pass
                else:
        ...
        # end of if action == 'sum':
        #
        # If we started out with a Numeric array, convert the MA to Numeric
        #
        if _NUM_FLAG:
            if MA.isMaskedArray(x):
                x = x.filled()
            # end of if MA.isMaskedArray(x):
            #
            if MA.isMaskedArray(retwts):
                retwts = retwts.filled()
            # end of if MA.isMaskedArray(retwts):
        # end of if _NUM_FLAG:
        #
        if returned:
            return x, retwts
        ...
        #
        return None
    # end of if MA.isMaskedArray(V):
    #
    #******************************************************************************************
    #
    # Case 3: Masked Variable.
    #
    if not weights: weights = 'weighted'
    #
    axis_order = []
    ...
    try:
        if __DEBUG__: print 'Are they equal?', MV.allclose(weights, area_weights(V,axisoptions))
    except:
        pass
    #
    if action == 'average':
